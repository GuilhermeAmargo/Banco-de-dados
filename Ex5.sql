...
SELECT nome_funcionario, nome_gerente FROM (funcionarios NATURAL INNER JOIN departamentos) INNER JOIN gerentes USING (id_gerente)
WHERE nome_departamento = 'Qualidade'

-- Encontrar o departamento em que um funcionário específico trabalha.
SELECT nome_departamento FROM (departamentos NATURAL INNER JOIN FUNCIONARIOS) INNER JOIN gerentes USING (id_gerente)
WHERE nome_funcionario = 'Ana Beatriz'
